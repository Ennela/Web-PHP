...
<?php
    ...
    if ( ! empty($_GET['action']) && $_GET['action'] == 'search'
        ...
        $_SESSION['locsanpham'] = $_POST;
        header('Location: quanlithanhvien.php');
        exit;
    }
    if ( ! empty($_SESSION['locsanpham'])) {
        $where = "";
        foreach ($_SESSION['locsanpham'] as $field => $value) {
        ...
        extract($_SESSION['locsanpham']);//tự động lấy ra biến id và hoten
    }
    $item_per_page = ! empty($_GET['per_page']) ? $_GET['per_page'] : 6;
    $current_page  = ! empty($_GET['page']) ? $_GET['page']
        : 1; //Trang hiện tại
    $offset        = ($current_page - 1) * $item_per_page;
    $totalRecords = mysqli_query($con, "SELECT * FROM `tbl_qlthanhvien`");
    $totalRecords = $totalRecords->num_rows;
    $totalPages   = ceil($totalRecords / $item_per_page);
//    ceil hàm làm tròn số
    if ( ! empty($where)) {
        $result = mysqli_query($con,
            "SELECT * FROM `tbl_qlthanhvien` where (".$where.") ORDER BY `id` ASC  LIMIT " .$item_per_page." OFFSET ".$offset);
    } else {
        $result = mysqli_query($con, "SELECT * FROM `tbl_qlthanhvien` ORDER BY `id` ASC  LIMIT ".$item_per_page." OFFSET ".$offset);
    }
...
<div class="flex flex-col md:flex-row">
    ...
    <div class="main-content flex-1 bg-gray-100 mt-12 md:mt-2 pb-24 md:pb-5">
        ...
        <div class="bg-gray-800 pt-3">
            <div class="page-hero">
                <h3 class="font-bold pl-2 text-white"><i class="far fa-address-book mr-2"></i> Danh sách thành viên</h3>
            </div>
        </div>
        ...
        <div class="flex flex-wrap">
            ...
            <div class="w-full p-6 animate-in">
                <!--Metric Card-->
                <div class="metric-card p-5">
                    <div class="flex flex-row items-center">
                        <div class="flex-shrink pr-4">
                            <div class="metric-icon"><i class="fas fa-user-tie fa-lg text-white"></i></div>
                        </div>
                        <div class="flex-1 text-right md:text-center">
                            <div class="metric-label">Tổng thành viên</div>
                            <div class="metric-value"><?= $totalRecords ?> <span class="metric-unit">thành viên</span></div>
                        </div>
                    </div>
                </div>
                <!--/Metric Card-->
            ...
            <div class="w-full px-6 pb-4 animate-in" style="animation-delay: 0.1s;">
                <div class="search-bar" style="justify-content: space-between;">
                    <form action="quanlithanhvien.php?action=search" method="POST" class="flex items-center gap-3 flex-1">
                        <input type="text"
                               placeholder="Mã thành viên ..." name="id"
                               value="<?=!empty($id)?$id:""?>"/>
                        <input type="text"
                               placeholder="Tên thành viên ..." name="hoten"
                    ...
                    <button class="btn-add">
                        <a href="./themthanhvien.php"><i class="fas fa-plus mr-1"></i> Thêm thành viên</a>
                    </button>
                </div>
            </div>
            
            <div class="flex w-full px-6 justify-end animate-in" style="animation-delay: 0.15s;">
                <div class="records-info"><i class="fas fa-info-circle" style="color: #3b82f6;"></i> Có tất cả <strong><?=$totalRecords?></strong> thành viên trên <strong><?=$totalPages?></strong> trang</div>
            </div>

            <div class="w-full p-6 animate-in" style="animation-delay: 0.2s;">
                    ...
                    <?php
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            <tr>
                                ...
                                <td class="text-center"><?= date('d/m/Y H:i',
                                        $row['ngaycapnhat']) ?></td>
                                <td class="text-center"><?= date('d/m/Y H:i',
                                        $row['ngaytao']) ?></td>
                                <td class="text-center "><?= $row['chucvu'] ?>
                                </td>
                                <td class="text-center doimau"><a
                                            href="./edit_qlthanhvien.php?id=<?= $row['id'] ?>"><i class="fas fa-edit mr-1"></i>Sửa</a>
                                </td>
                                <td class="text-center doimau"><a
                                            href="./delete_qlthanhvien.php?id=<?= $row['id'] ?>"><i class="fas fa-trash-alt mr-1"></i>Xóa</a>
                                </td>
                            </tr>
                            <?php
                        } ?>
