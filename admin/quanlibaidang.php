...
<?php
    ...
    if (!empty($_GET['action']) && $_GET['action'] == 'search' && !empty($_POST)) {
        $_SESSION['locsanpham4'] = $_POST;
        header('Location: quanlibaidang.php');
        exit;
    }
    if (!empty($_SESSION['locsanpham4'])) {
        $where = "";
        foreach ($_SESSION['locsanpham4'] as $field => $value) {
        ...
        extract($_SESSION['locsanpham4']);//tự động lấy ra biến id và hoten
    }
    ...
    if (!empty($where)) {
        $result = mysqli_query($con,
            "SELECT * FROM `tbl_qlbaidang` where (" . $where . ") ORDER BY `id` ASC  LIMIT " . $item_per_page .
            " OFFSET " . $offset);
    } else {
        $result = mysqli_query($con,
...
<div class="flex flex-col md:flex-row">
    ...
    <div class="main-content flex-1 bg-gray-100 mt-12 md:mt-2 pb-24 md:pb-5">
        ...
        <div class="bg-gray-800 pt-3">
            <div class="page-hero">
                <h3 class="font-bold pl-2 text-white"><i class="fas fa-align-left mr-2"></i> Danh sách bài viết</h3>
            </div>
        </div>
        ...
        <div class="flex flex-wrap">
                ...
                <div class="metric-card p-5">
                    <div class="flex flex-row items-center">
                        <div class="flex-shrink pr-4">
                            <div class="metric-icon"><i class="fas fa-newspaper fa-lg text-white"></i></div>
                        </div>
                        <div class="flex-1 text-right md:text-center">
                            <div class="metric-label">Tổng bài viết</div>
                            <div class="metric-value"><?= $totalRecords ?> <span class="metric-unit">bài viết</span></div>
                        </div>
                    </div>
                </div>
                <!--/Metric Card-->
            ...
            <div class="w-full px-6 pb-4 animate-in" style="animation-delay: 0.1s;">
                <div class="search-bar" style="justify-content: space-between;">
                    <form action="quanlibaidang.php?action=search" method="POST" class="flex items-center gap-3 flex-1">
                        <input type="text"
                               placeholder="Mã bài đăng ..." name="id"
                               value="<?= !empty($id) ? $id : "" ?>"/>
                        <input type="text"
                               placeholder="Tên bài đăng ..." name="tieude"
                               value="<?= !empty($tieude) ? $tieude : "" ?>"/>
                        <button type="submit" class="btn-search">
                            <i class="fas fa-search"></i> Tìm kiếm
                        </button>
                    </form>
                    <button class="btn-add">
                        <a href="./thembaidang.php"><i class="fas fa-plus mr-1"></i> Thêm bài đăng</a>
                    </button>
                </div>
            </div>
            <div class="flex w-full px-6 justify-end animate-in" style="animation-delay: 0.15s;">
                <div class="records-info"><i class="fas fa-info-circle" style="color: #3b82f6;"></i> Có tất cả <strong><?= $totalRecords ?></strong> bài viết trên <strong><?= $totalPages ?></strong> trang</div>
            </div>
                    ...
                    <?php
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            <tr>
                                ...
                                <td class="text-center"><?= $row['nguoiphutrach'] ?></td>
                                <td><img height="100"
                                         src="<?= $row['anhdaidien'] ?>"
                                         width="100"/></td>
                                <td class="text-center"><?= $row['chedo'] ?></td>
                                ...
                                <td class="text-center doimau"><a
                                            href="./edit_qlbaidang.php?id=<?= $row['id'] ?>"><i class="fas fa-edit mr-1"></i>Sửa</a>
                                </td>
                                <td class="text-center doimau"><a
                                            href="./delete_qlbaidang.php?id=<?= $row['id'] ?>"><i class="fas fa-trash-alt mr-1"></i>Xóa</a>
                                </td>
                            </tr>
                            <?php
                        } ?>
